...
WP_CLI::line( 'CONTACT_SHORTCODES=company:' . substr_count( $contact_source, '[kuka_company_details]' ) . '|support:' . substr_count( $contact_source, '[kuka_contact_details]' ) . '|form:' . substr_count( $contact_source, '[kuka_contact_form]' ) );
...
$product_source = (string) file_get_contents( get_stylesheet_directory() . '/assets/js/product.js' ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_get_contents_file_get_contents
$product_css_source = (string) file_get_contents( get_stylesheet_directory() . '/assets/css/product.css' ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_get_contents_file_get_contents
$product_gallery_source = (string) file_get_contents( get_stylesheet_directory() . '/woocommerce/single-product/product-image.php' ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_get_contents_file_get_contents
...
$community_source = (string) file_get_contents( get_stylesheet_directory() . '/template-parts/community-gallery.php' ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_get_contents_file_get_contents
$community_css_source = (string) file_get_contents( get_stylesheet_directory() . '/assets/css/community.css' ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_get_contents_file_get_contents
...
WP_CLI::line( 'COMING_SOON_VIDEO_CACHE_BUSTED=' . ( str_contains( $front_source, 'kuka_island_child_asset_url' ) && str_contains( $splash_source, 'kuka_island_child_asset_url' ) ? 'home+coming-soon' : 'missing' ) );
...
WP_CLI::line( 'PRODUCT_LIGHTBOX_POINTERS=' . ( str_contains( $product_source, 'zoomFromPoint(event, image)' ) && str_contains( $product_source, '[data-lightbox-viewport], [data-lightbox-image-host]' ) && str_contains( $product_source, '[data-panel-close]' ) && str_contains( $product_source, 'suppressImageClick' ) && str_contains( $product_css_source, '--zoom-origin-x' ) ? 'point-zoom+backdrop-close+drag-guard' : 'incomplete' ) );
WP_CLI::line( 'PRODUCT_DETAIL_IMAGE_QUALITY=' . ( str_contains( $product_gallery_source, "wp_get_attachment_image_src( \$image_id, 'full' )" ) && str_contains( $product_gallery_source, "'sizes'         => '(max-width: 47.99em) 100vw, 67vw'" ) && ! str_contains( $product_gallery_source, "wp_get_attachment_image_src( \$image_id, 'large' )" ) ? 'responsive-full' : 'undersized' ) );
WP_CLI::line( 'COMMUNITY_PRODUCT_CTA=' . ( str_contains( $community_source, "'View product' : 'Ürüne Git'" ) && str_contains( $community_source, 'class="kuka-community__cta"' ) && str_contains( $community_css_source, '.kuka-community__cta' ) ? 'bilingual+linked-only' : 'missing' ) );
...
WP_CLI::line( 'STORY_MENU_LABEL=' . ( in_array( 'HİKAYEMİZ', $menu_rows, true ) ? 'HİKAYEMİZ' : 'missing' ) );
