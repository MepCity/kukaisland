...
expect_line "measured Site Appearance inventory" "SITE_APPEARANCE_INVENTORY=14_groups|118_rows|161_controls"
...
expect_line "43 bilingual appearance fields" "LANGUAGE_TRANSLATABLE_FIELDS=43"
expect_line "nine English product fields" "PRODUCT_EN_FIELD_SCHEMA=9"
expect_line "three English page fields" "PAGE_EN_FIELD_SCHEMA=3"
...
expect_line "all Site Appearance English fields seeded" "SITE_APPEARANCE_EN_VALUES=43/43"
expect_line "array-backed translated labels render normally" "TRANSLATED_LABEL_ARRAYS=supported"
expect_line "navigation translations retain all five labels" "NAVIGATION_LABEL_CAP=5"
...
expect_shipping_match "the OpenAPI documents match their checksums and the client matches them" "^DHL_OPENAPI_CONTRACT=(PASS\\|checksums:6/6\\|documents:6\\|operations_declared:[0-9]+\\|operations_used:14\\|status_codes:8\\|host:pinned\\|base_paths:matched|SKIPPED\\|reason:(spec_directory_absent|python3_absent))"
...
expect_shipping_match "only the sandbox endpoints may be contacted" "^SHIPPING_ENDPOINT_ALLOWLIST=PASS\\|cases:17\\|wrong:none$"
...
expect_shipping_match "the documented object and measured one-item query envelopes are both accepted" "^SHIPPING_QUERY_LIST_ENVELOPES=PASS\\|getorder:success\\|getshipmentstatus:success\\|status_code:1\\|documented_object_and_sandbox_list:accepted$"
...
expect_shipping_match "the fulfilment record carries provider dhl and the carrier own shipment id" "^SHIPPING_FULFILLMENT_RECORD=PASS\\|record:created\\|provider_key:dhl\\|tracking_number:set\\|is_the_shipment_id:yes\\|is_a_label:no\\|status_on_create:unfulfilled$"
expect_shipping_match "the tracking number is the shipment id and never a label" "^SHIPPING_TRACKING_NUMBER_SOURCE=PASS\\|default:shipment_id\\|shipment_id:selectable\\|barcode:refused_to_unmeasured\\|.*contract_constants_identical:yes\\|adapter_answers_contract:yes$"
...
expect_shipping_match "an inconclusive reconciliation keeps both doors shut" "^SHIPPING_INCONCLUSIVE_STAYS_SHUT=PASS\\|createbarcode_attempts:1\\|createOrder_attempts:1\\|state:reconcile_required\\|create_again:already_in_progress\\|resume_again:not_resumable$"
...
expect_shipping_match "every state but the two cancellable ones sends nothing" "^SHIPPING_CANCEL_REFUSES_EVERY_OTHER_STATE=PASS\\|states_checked:12\\|wrong:none\\|carrier_writes:0\\|unknown_state_refused:yes\\|shipment_created_without_id_refused:yes$"
...
expect_shipping_match "every state but the two amendable ones sends nothing" "^SHIPPING_UPDATE_REFUSES_EVERY_OTHER_STATE=PASS\\|states_checked:11\\|wrong:none\\|carrier_writes:0$"
...
expect_shipping_match "the automatic path keeps the two phases in separate worker turns" "^SHIPPING_AUTO_CREATE_IS_TWO_PHASE=PASS\\|measured:three_real_dispatcher_turns_and_mock_transport\\|first:createRecipient=1/createOrder=0/createbarcode=0/state=recipient_created/follow_up_jobs=1/gap=[1-9][0-9]*\\|second:createOrder=1/state=order_created/follow_up_jobs=1\\|third:createbarcode=1/state=shipment_created/ok=yes\\|phases:create_barcode$"
expect_shipping_match "an uncertain first phase is not retried" "^SHIPPING_AUTO_CREATE_PHASE_ONE_CRASH_IS_SAFE=PASS\\|.*\\|retry:createRecipient=0/writes=0/"
expect_shipping_match "a local recipient refusal sends nothing and is not retried" "^SHIPPING_AUTO_CREATE_RECIPIENT_LOCAL_REFUSAL_IS_SAFE=PASS\\|.*\\|first_writes=0/state=none/pending=absent\\|later_writes=0/"
...
expect_shipping_match "both create doors are an allow-list, and no other state reaches them" "^SHIPPING_CREATE_DOORS_ARE_AN_ALLOWLIST=PASS\\|measured:which_door_opened\\|states:13\\|actions:2\\|createOrder_allowed_from:none\\+blocked\\+absent_confirmed\\+recipient_created\\|createbarcode_allowed_from:order_created\\|wrong:none\\|carrier_writes:0$"
...
expect_shipping_match "the SMTP credential is not in the admin surface or the database" "^SHIPPING_SMTP_SECRET_STAYS_OUT_OF_THE_DATABASE=PASS\\|measured:source_and_options_table\\|password_input_fields:0\\|registered_settings:0\\|smtp_option_rows:0\\|constants_from_wp_config:6/6\\|runtime_state:(configured|unconfigured)\\|host_files:measured_by_SMTP_SECRET_TRANSPORT$"
...
expect_shipping_match "one outcome, one save" "^SHIPPING_MUTATION_OUTCOME_ATOMIC=PASS\\|measured:order_store_save_counter\\|transitions:cancel_confirmed:1,update_confirmed:1,update_mismatch:1,intent_opened_and_restored:2,create_and_barcode_confirmed:5\\|wrong:none$"
...
expect_shipping_match "the behavioural suite removes its fixtures and its notes and preserves the cache" "^SHIPPING_FIXTURES_REMOVED=PASS\\|remaining_fixture_orders:0\\|order_note_delta:0\\|cache_rows_restored:yes\\|cache_rows_left:[0-9]+\\|run_owned_cache_removed:[0-9]+\\|cache_release_refused:0\\|sentinels_removed_by_exact_name:4\\|action_group_row:(removed|preexisting|absent)$"
...
expect_shipping_match "the carrier budget and the local retry counter stay separate" "^SHIPPING_SYNC_COUNTERS_AND_DELAY_ARE_SEPARATE=PASS\\|measured:real_action_scheduler_row\\|returned_attempts:7\\|returned_sync_attempts:1\\|stored_query_attempts:7\\|stored_sync_attempts:1\\|first_delay_seconds:1[0-4][0-9]$"
expect_shipping_match "a refusal no retry can fix is recorded, shown and noted once" "^SHIPPING_SYNC_NONRETRYABLE_IS_VISIBLE_ONCE=PASS\\|measured:real_manager_path\\|reason:claim_other_record\\|pending_retry:0\\|notes_after_first:1\\|notes_after_repeat:1\\|panel_reason:yes\\|panel_manual:yes\\|mails:0\\|reference_missing_outcome:claim_reference_missing$"
expect_shipping_match "a successful sync clears the local bookkeeping" "^SHIPPING_SYNC_BOOKKEEPING_CLEARS_ON_SUCCESS=PASS\\|measured:real_manager_path\\|reason_before:claim_lock_contended\\|reason_after:cleared\\|attempts_after:0\\|schedule_error_after:cleared\\|mails:1$"
expect_shipping_match "a terminal first carrier answer still finishes the notification" "^SHIPPING_TERMINAL_FIRST_ANSWER_STILL_NOTIFIES=PASS\\|measured:real_action_scheduler_worker_and_intercepted_transport\\|carrier_writes:0\\|cases:3\\|claim_lock_contended:ok/lifecycle:delivered/claim:claim_lock_contended/fulfilled:no/date:absent/mails:0/follow_up:1/runner:stop:terminal_lifecycle:fulfillment_retry:created/retry_fulfilled:yes/retry_mails:1/total_mails:1\\|notification_claim_unverified:ok/.*\\|claim_order_unreadable:ok/lifecycle:delivered/claim:claim_order_unreadable/.*total_mails:1$"
...
expect_shipping_match "a pending notification is retried once the missing condition is fixed" "^SHIPPING_NOTIFICATION_PENDING_RETRIES_WHEN_DUE=PASS\\|first:recipient_missing\\|state_after_first:pending\\|attempts_after_first:0\\|mails_first:0\\|second_first_transition:no\\|second:sent\\|mails_second:1\\|state:sent\\|attempts:1\\|third:already_sent\\|mails_total:1\\|order_status:pending$"
expect_shipping_match "a process killed between the save and the send intent still notifies once" "^SHIPPING_NOTIFICATION_CRASH_BEFORE_SEND_INTENT_RECOVERS=PASS\\|crash_exit:nonzero\\|crash_mails:0\\|record_fulfilled_after_crash:yes\\|state_after_crash:due\\|recovery:sent\\|mails_total:1\\|state:sent\\|attempts:1\\|second_automatic_send:0\\|order_status:pending$"
expect_shipping_match "a contended claim lock writes nothing and sends nothing" "^SHIPPING_NOTIFICATION_CLAIM_LOCK_IS_FAIL_CLOSED=PASS\\|lock_held_by_other_session:yes\\|outcome:claim_lock_contended\\|notification_meta_writes:0\\|fulfilled:no\\|date:absent\\|mails:0\\|retry_outcome:sent\\|retry_mails:1\\|retry_fulfilled:yes\\|date_from_first_successful_claim:yes$"
expect_shipping_match "a claim whose write did not land is refused, not assumed" "^SHIPPING_NOTIFICATION_CLAIM_READBACK_IS_VERIFIED=PASS\\|outcome:notification_claim_unverified\\|notification_meta_writes:0\\|fulfilled:no\\|date:absent\\|mails:0\\|retry_outcome:sent\\|retry_mails:1\\|retry_fulfilled:yes$"
expect_shipping_match "an order that cannot be re-read stops the dispatch write" "^SHIPPING_NOTIFICATION_CLAIM_UNREADABLE_ORDER_IS_FAIL_CLOSED=PASS\\|outcome:claim_order_unreadable\\|notification_meta_writes:0\\|fulfilled:no\\|date:absent\\|mails:0$"
expect_shipping_match "a record nobody claimed is never adopted for a notification" "^SHIPPING_NOTIFICATION_NO_SELF_ADOPTION=PASS\\|state_before:absent\\|record_fulfilled:yes\\|outcome:not_due\\|mails:0\\|state_after:absent\\|attempts:0$"
expect_shipping_match "the handover date follows the agreed instant, not either process clock" "^SHIPPING_FULFILLMENT_DATE_FOLLOWS_AGREED_INSTANT=PASS\\|concurrent_date_writes:2\\|offset_process_date_writes:1\\|seeded_local_date_differs_from_today:yes\\|stored_matches_agreed:yes\\|retry_moves_date:no\\|mails:1\\|edm_shipment_date_can_differ:no$"
...
expect_invoice_match "the transmission gate seam defaults to the real gate and never weakens it" "^EDM_TRANSMISSION_GATE_SEAM=PASS\\|measured:constructed_objects_and_production_send_path_intercepted\\|production_default:Kuka_Island_Core_Invoice_Runtime_Gate\\|open_gate_consulted:1\\|open_gate_SendInvoice:1\\|closed_gate_consulted:1\\|closed_gate_code:edm_runtime_disabled\\|closed_gate_SendInvoice:0\\|closed_gate_uuid:absent\\|production_sites_passing_a_gate:0\\|open_error:none$"
...
# The invoice suite reads ONE local, content-pinned WSDL fixture and posts
# nothing. Both halves are pinned here, because "we believe it is offline" is
# what let a network-dependent suite sit in this gate for months.
expect_invoice_match "the invoice verification reads a local WSDL and sends nothing" "^INVOICE_VERIFY_NETWORK_ISOLATED=PASS\\|measured:fixture_loader_wp_http_layer_and___doRequest\\|wsdl_source:local_file\\|wsdl_reads:[0-9]+\\|all_reads_local_file:yes\\|wsdl_sha256_pinned:yes\\|fixture_names_an_edm_host:no\\|edm_endpoint_posts:0\\|edm_http_requests_attempted:0\\|production_send_path_intercepted:[1-9][0-9]*\\|SendInvoice_through_production_client:[1-9][0-9]*\\|mock_responses_used:[1-9][0-9]*\\|soap_endpoints_offered:http://edm-offline-fixture.invalid/EFaturaEDM.svc$"

# Omitting EDM's own GIB report dates is impossible against the pinned WSDL
# fixture: the encoder refuses before any transport. BLOCKED is the honest
# verdict, and the whole line is pinned so a relaxed WSDL breaks this and forces
# a re-measure.
expect_invoice_match "omitting EDM's GIB report dates is refused by the encoder" "^INVOICE_OUTGOING_REQUEST_OMITS_REPORT_SENDDATES=BLOCKED\\|measured:pinned_wsdl_fixture_soap_encoder\\|network_soap_operations:0\\|omission_verdict:SoapFault\\|omission_envelope_produced:no\\|encoder_message:SOAP-ERROR: Encoding: object has no 'EARCHIVE_REPORT_SENDDATE' property\\|control_serialises:yes\\|control_senddate_nodes:1,1\\|"
...
expect_invoice_line "SendInvoice e-Archive DOMXPath and single base64" "INVOICE_SOAP_XPATH_SEND_INVOICE_EARCHIVE=PASS|assertions:26|single_base64_sha256_match:yes|internetsalesdetails_nodes:1|tuzelKisi_nodes:1|gercekKisi_nodes:0|specified_elements:none|error:none|failed:none"
...
expect_line "measured Coming Soon video bytes" "COMING_SOON_VIDEO_BYTES=desktop:13660330|mobile:7136721"
expect_line "versioned direct video URLs" "COMING_SOON_VIDEO_CACHE_BUSTED=home+coming-soon"
