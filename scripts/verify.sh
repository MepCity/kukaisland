...
expect_shipping_match "an absent shipment never proves a cancelled order" "^SHIPPING_CANCEL_ORDER_NOT_CANCELLED_ON_SHIPMENT_404=PASS\\|cancel_order:success\\|read_shipment:not_found\\|read_order:present\\|cancelorder_calls:1\\|getorder_calls:1\\|getshipment_calls:0\\|code:cancel_unconfirmed_record_present\\|state:cancel_reconciliation_required\\|cancelled_written:no$"
...
expect_shipping_match "the translation catalogue matches the source exactly" "^SHIPPING_POT_CATALOG=PASS\\|pot:readable\\|source_literals:[0-9]+\\|catalog_msgids:[0-9]+\\|missing_from_catalog:0\\|stale_in_catalog:0\\|required_new_strings:31/31\\|retired_hardcoded_carrier_string:removed$"
...
expect_shipping_match "a definitive refusal is the one answer that keeps the old state" "^SHIPPING_CANCEL_DEFINITIVE_REFUSAL_KEEPS_STATE=PASS\\|refusal:permanent\\|code:bad_request\\|state_after_refusal:shipment_created\\|evidence_recorded:no\\|retry_allowed:yes\\|state:cancelled\\|cancel_writes:2$"
...
expect_shipping_match "an exact field read-back is what proves an amendment" "^SHIPPING_UPDATE_EVIDENCE_READBACK_MATCHES=PASS\\|readback:exact_match\\|fields_compared:9\\|code:update_confirmed\\|state:shipment_created\\|evidence:cleared\\|read_amendable_fields:1\\|second_update_allowed:yes\\|update_writes:2$"
