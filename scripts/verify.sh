...
expect_shipping_match "the translation catalogue matches the source exactly" "^SHIPPING_POT_CATALOG=PASS\\|pot:readable\\|source_literals:[0-9]+\\|catalog_msgids:[0-9]+\\|missing_from_catalog:0\\|stale_in_catalog:0\\|required_new_strings:47/47\\|retired_hardcoded_carrier_string:removed$"
...
expect_invoice_match "the EDM run-gate option is put back byte-for-byte on every exit" "^EDM_RUNTIME_OPTION_RESTORED=PASS\\|measured:options_table_row\\|exists:yes\\|value_len:1\\|autoload:off\\|byte_equivalent:yes$"
