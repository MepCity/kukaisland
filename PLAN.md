...
| 2026-09-14 | Kargo panelinin üç sınırı ölçülerek sertleştirildi: kayıtlı kimlik bilgisini silme onayı **sunucuda** tam değer karşılaştırmasıyla doğrulanıyor (HTML `required` sınır değildir); otomatik gönderinin retry bütçesi gerçek hâle getirildi ve **faz-farkında** oldu (`none` → createOrder, `order_created` → createbarcode); otomatik yolun gönderdiği her faz çağrıdan önce siparişe işaretleniyor ve yalnız yapısal kanıt "gönderilmedi" dediğinde geri alınıyor. Retry uygunluğu tek merkezî politikada: ilan edilmiş kod listesi **ve** siparişin kendi satırından yapısal kanıt; HTTP kodundan çıkarım yok. Planlama Action Scheduler satırı geri okunarak doğrulanıyor. Dispatcher'ın planlama kilidinin (`kuka_ship_query_`) mutasyon kilidinden (`kuka_ship_mutate_`) ayrı olduğu belgelerde düzeltildi | Sayacın var olması harcandığını kanıtlamaz: `retry_budget_spent` normal akışta ulaşılamazdı ve kilit çekişmesiyle düşen sipariş sessizce hiç gönderilmiyordu. Ölçüm ayrıca üçüncü bir şeyi açığa çıkardı: salt-okunur mutabakat siparişi meşru biçimde `order_created`'a geri koyabiliyor ve yalnız duruma bakan bir worker bunu ikinci bir `createbarcode` için izin sayıyordu. "Panel bağlantı testi ana kapıyı atlıyor" iddiası için yazılan RED ise GREEN çıktı — garanti adaptörün `preflight()` yolunda zaten vardı; ölçüm sözleşme olarak bırakıldı ve panel kapıyı adaptörden bağımsız olarak da soruyor |
| 2026-09-14 | Kargo entegrasyonu müşteriye teslim edilebilir hâle getirildi: WooCommerce altında "Kargo Entegrasyonu" ayar sayfası, dört ayrı anahtar (modül/adaptör/otomatik gönderi/otomatik sorgu), salt-okunur bağlantı testi ve isteğe bağlı otomatik gönderi oluşturma. Sırlar düz option'a **yazılmaz**; `sodium_crypto_secretbox` ile, wp-config salt'larından HKDF ile türetilen ve saklanmayan bir anahtarla, `autoload=no` tek satırda şifreli durur. Öncelik `wp-config sabiti > ortam değişkeni > panel kasası`. Otomatik gönderi **varsayılan kapalı**, checkout isteği içinde taşıyıcı aramaz, uygunluk izin listesidir, iki faz ayrı kalır ve belirsiz yazma tekrarlanmaz. Canlı ortam panelden seçilebilir fakat yapısal olarak bloke | Site sahibi `wp-config.php` düzenleyemez; seçenek "sırları dikkatle saklayan bir panel" ile "hiç panel" arasındaydı ve ikincisi daha güvenli değil, yalnızca yapılandırılmamış bir entegrasyon demekti. Eski "sır asla option'a yazılmaz" kuralı yasak olmaktan çıkıp öncelik oldu; düz option hâlâ yasak. 39 yeni davranış ölçümü (`scripts/verify-shipping-admin-settings.php`), otomatik/manuel yarışı **gerçek iki PHP süreci ve ayrı MySQL oturumuyla** ölçülüyor: süreçler arası toplam taşıyıcı yazması 1 |
| 2026-09-05 | E-posta kabul ölçümleri SMTP hesabı olmayan ortamı geçerli sonuç sayar: kapalı `mail()` ölçümünde Core PHPMailer’a inmeden reddederse `DISABLED_MAIL_TRANSPORT=none:refused_before_transport`, şablon sır ölçümünde tanımsız kullanıcı adı `username_is:undefined`. `private_credential` ve SMTP yapılandırılmışken `unknown` hâlâ FAIL | GitHub Actions’ta SMTP hesabı yoktur ve olmamalıdır; K-48 ölçümleri geliştirici ortamındaki gerçek hesabı varsaydığı için CI dört gündür kırmızıydı ve arkasındaki gerçek hataları gizliyordu |
| 2026-09-05 | Vitrin SEO katmanı tema `inc/seo.php` içinde tek yerden basılır: meta açıklaması (ürün metası → panel `seo` grubu → kategori açıklaması → sayfa metası; boş alan etiket üretmez), site geneli Open Graph/Twitter kartı (ürün sayfasında ürün görseli, aksi hâlde panel paylaşım görseli), yalnız ana sayfada panel verisinden `OnlineStore` JSON-LD ve hero fotoğrafı için `wp_preload_resources` ile mobil/masaüstü ayrı preload. Blocksy’nin katalog hero’su `blocksy:hero:custom-source` filtresiyle kapatılır; sepet ikonunun erişilebilir adı görünür sayacı içerir; `content.css` ana sayfada yüklenmez | Lighthouse yerel mobil ölçümü: ana sayfa ve katalogda meta açıklaması yoktu, katalogda iki `h1` vardı, sepet ikonunda görünür metin ile `aria-label` uyuşmuyordu, hero arka plan görseli `style` içinde olduğu için geç keşfediliyordu. Açıklama metinleri koda gömülmez, operatör panel/sayfa/kategori alanından yönetir |
| 2026-09-05 | Hero ve “çok yakında” videosu `load` sonrasında `requestIdleCallback` ile başlar; `navigator.connection.effectiveType` 4g dışını bildirirse hiç indirilmez ve poster kalır. Dosyalar, poster, döngü ve `preload="none"` değişmedi; Safari/Firefox bağlantı türünü bildirmediği için orada video her bağlantıda oynar | Mobil emülasyonda sayfa açılır açılmaz 6,5 MB video hero fotoğrafıyla bant genişliği paylaşıyor ve LCP 7,9 s ölçülüyordu. Kaliteyi düşürmeden ilk ekranı hızlandırmanın tek yolu videonun ne zaman ve kime indiğini değiştirmekti; ölçüm sonuçları §39’da |
| 2026-09-03 | Kargo otomasyonu Core'a değil, `kuka-island-shipping-automation` adlı ayrı ve varsayılan pasif eklentiye kondu; taşıyıcı sözleşmesi soyut, ilk adaptör DHL eCommerce Türkiye. Canlı ortam ve kapıda ödeme bloke; otomasyon varsayılan kapalı; gönderi yalnız operatörün açık işlemiyle oluşur | Kargo firması kararı henüz kesin değil (§39). Core'a gömülen bir taşıyıcı, firma değişirse Core'u değiştirmeyi gerektirirdi. İkinci taşıyıcı `kuka_island_shipping_carriers` filtresine bir adaptör eklenerek gelir; Core, Manager, Poller, Admin ve manuel fulfillment akışı değişmez |
| 2026-09-03 | Taşıyıcı yanıtları dört sonuç türüne ayrıldı: `success`, `permanent`, `transient`, `uncertain`. Belirsiz bir yazma **hiçbir koşulda** tekrarlanmaz; yokluk yalnız `getshipment` ve `getorder` sorgularının ikisinin de `not_found` demesiyle kanıtlanır | Timeout, okunamayan 2xx, 409, 429 ve 5xx "gönderilmedi" anlamına gelmez. Belirsizliği geçici hata saymak aynı paketi iki kez kargolar; kapalı kalmak ise ücretsizdir |
| 2026-09-03 | Taşıyıcı yanıtındaki hangi değerin WooCommerce takip numarası olduğu ölçülmeden yazılmaz; `KUKA_DHL_TRACKING_NUMBER_SOURCE` tanımsızken alan boş bırakılır ve siparişe not düşülür | `createbarcode` hem `shipmentId` hem parça barkodları döner. Takip etmeyen bir numara müşteri e-postasına ve destek konuşmalarına girer; boş alan bundan iyidir |
| 2026-09-03 | EDM entegrasyonu Core'dan ayrılıp `kuka-island-edm` adlı, varsayılan pasif eklentiye taşındı; bağımlılık yalnız EDM → Core yönündedir. Deploy eklentiyi taşır ama etkinleştirmez; deaktivasyon çalışma kapısını önce kapatır ve yalnız kendi bekleyen action'larını temizler | Müşteri belirli sipariş hacmine ulaşana kadar faturayı manuel kesecek. Mali entegrasyonun pasifken hiçbir sınıf/hook/panel/SOAP/action/meta üretmemesi, manuel WooCommerce akışını bozmaması ve ileride kontrollü etkinleştirilebilmesi gerekir |
| 2026-09-03 | EDM testinde doğrulanan belge sözleşmesi: bireysel e-Arşiv `11111111111` + gerçek ad/soyad; `cac:Person`, `cac:Contact` sonrasında; müşteri e-postası UBL ve `HEADER.TO` alanlarında; e-Arşiv `RECEIVER.alias` yok; UBL numara sentinel'i `ABC2009123456789`, gerçek numara yalnız EDM yanıtından | İlk gerçek `SendInvoice`, `cac:Person` yanlış sırada olduğu için reddedildi; EDM desteğinin request incelemesi kök nedeni kesinleştirdi. Düzeltilmiş tek çağrı kabul edildi, numaralandı ve son ölçümde `PACKAGE - PROCESSING` kaldı; terminal `SEND - SUCCEED` görülmeden nihai başarı iddia edilmez |
| 2026-09-03 | Gönderim kanıtı bulunan belgede kör yeniden gönderim yoktur; durum sorgusu ayrı poller action'ına, ön-gönderim retry'ı ayrı üç denemelik kuyruk sayacına aittir. `queued` worker için geçerli, operatör resend'i için geçersizdir | Timeout veya belirsiz yanıt “gönderilmedi” anlamına gelmez. Send kuyruğunun sorguyu sahiplenmesi ve mali attempt sayacını retry bütçesi olarak kullanması sonsuz zincir/mükerrer belge riski yaratıyordu |
...
| 2026-09-05 | Ana sayfa ve “Çok yakında” ekranının ortak responsive video çifti yüksek kaliteye geçirildi | Önceki 1080p masaüstü `1,45 Mbps` ve 720p mobil `0,70 Mbps` sürümler hareketli kumaş/bitki detaylarını belirgin eritiyordu. Masaüstünde Git geçmişindeki en iyi kaynak byte olarak aynen kullanılıyor (`1920×1080 / 4,37 Mbps / 13.660.330 bayt`, kaynak karşısı SSIM `1,0000`); mobil dengeli biçimde yeniden kodlandı (`900×1600 / 2,28 Mbps / 7.136.721 bayt`, SSIM `0,9822→0,9920`). Yalnız viewport'a uygun dosya yüklenir; `preload=none`, Save-Data ve reduced-motion poster kapıları korunur |
...
- [x] Otomatik worker'ın defteri doğrulanıyor: başlangıç yazması sabote edildiğinde taşıyıcı yazması `0`, retry `0`, neden `dispatch_intent_unverified`; faz kaldırma yazması sabote edildiğinde ikinci yazma `0`, retry `0`, neden `dispatch_phase_clear_unverified`; sipariş taze okunamazsa `dispatch_order_unreadable` ve deneme harcanmıyor
- [x] Defterin güvencesi **atomiklik değil**, geri okumadır: tek persist turu birden çok SQL ifadesi çıkarabilir ve deneme sayısı ile faz işareti yarım inebilir. "Tek atomik başlangıç" / "yarıya inemezler" ifadeleri Order_Store yorumundan, GECMIS §15.8'den, K-68'den, DHL_ENTEGRASYONU §17.2'den ve bu defterden kaldırıldı
- [ ] Yarım-yazma iki yönü ayrı ayrı ölçüme bağlandı — sayaç iniyor/faz işareti düşürülüyor (`d_attempt_landed_phase_dropped`) ve faz işareti iniyor/sayaç düşürülüyor (`e_phase_landed_attempt_dropped`); ikisinde de beklenen taşıyıcı yazması `0`, retry `0`, neden `dispatch_intent_unverified`. Beklenen çıktı `scripts/verify.sh` içinde sabit; **ölçüm sonucu henüz kaydedilmedi**
- [x] İki otomatik worker gerçek iki PHP süreciyle yarıştırıldı: `createOrder_across_processes:1`, `createbarcode_across_processes:1`, kaybeden `dispatch_in_progress` ile yazmadan çıkıyor, `attempts_total:1`, faz kaydı korunuyor, yürütme kilidi sonunda serbest
- [x] Üç kilit kaynakta ve belgelerde birebir aynı: planlama `kuka_ship_query_`, worker yürütme `kuka_ship_dispatch_`, taşıyıcı mutasyonu `kuka_ship_mutate_`

- [x] Kayıtlı kimlik bilgisini silme fail-closed: onay yok / yanlış / boş → `secret=kept`; yalnız tam `'1'` değeriyle siliniyor ve diğer üç bilgi değişmiyor. Yanlış nonce ve yetersiz yetki davranışları korunuyor
- [x] Otomatik gönderi retry'ı gerçek ve faz-farkında: createOrder öncesi çekişmede tek retry planlanıp engel kalkınca `createOrder=1|createbarcode=1`; createbarcode öncesi çekişmede `createOrder` tekrarlanmadan faz iki sürüyor; belirsiz `createOrder`/`createbarcode` sonrasında retry `0` ve ikinci yazma `0`; bütçe 3 turda tükeniyor; planlanamayan retry "planlandı" diye raporlanmıyor; anahtar kapatılırsa retry `0` ve taşıyıcı çağrısı `0`
- [x] Ana çalışma kapısı kapalıyken panel bağlantı testi: `identity_calls:0|cbs_calls:0|total_http:0|code:shipping_runtime_disabled`, sır sızıntısı `0`
...
- [x] Otomatik gönderi oluşturma eklendi ve **varsayılan kapalı**: ödeme kancası taşıyıcı aramaz, tek Action Scheduler işi planlar; üç olay tek iş üretir; uygunluk dokuz koşullu izin listesi; iki faz ayrı (`createOrder` → taze DB okuması → `createbarcode`); belirsiz yazma otomatik tekrarlanmaz; deneme bütçesi 3 ve panelde görünür
...
- [x] SEO katmanı: meta açıklaması TR/EN ana sayfa, mağaza, ürün ve sayfalarda basıldı (kategori sayfası kategori açıklaması boşken etiket basmaz); site geneli `og:type/site_name/title/description/url` ve Twitter kartı, ürün sayfasında `og:image` = ürün görseli; ana sayfada `OnlineStore` JSON-LD geçerli JSON (`name,url,email,telephone,contactPoint,sameAs,address`); hero için iki `preload` (mobil `max-width: 47.5em`, masaüstü `min-width: 47.5625em`, `fetchpriority=high`); katalog `h1` 2→1; sepet ikonu erişilebilir adı `Sepeti aç 0` / `Open cart 0`; ana sayfada `content.css` 0. Panel envanteri `14 grup / 118 satır / 161 kontrol`, çift dilli alan `43/43`, sayfa EN alanı `3`; PHPCS 145/145, smoke `5/5`
- [x] Video erteleme ölçümü (Lighthouse 12, yerel Docker, mobil emülasyon, simüle yavaş 4G): ana sayfa perf `68→74`, LCP `7,9→7,2 s`, aktarım `7,8→5,8 MB`; katalog perf `83→96`, LCP `4,3→2,6 s`; ürün `82` değişmedi. Kalan ana sayfa LCP yükü 430 KB mobil hero JPEG ve `<video poster>` ile mobilde de inen 174 KB masaüstü posteri; ikinci tur: responsive hero görseli ve tek poster kararı
- [ ] Lansman öncesi operatör girdisi: Site Görünümü → 14. SEO metinleri, sayfa düzenleyici meta açıklamaları, kategori açıklamaları, Marka → sosyal paylaşım görseli ve Ayarlar → Genel → Slogan doldurulacak; `blog_public=1` ile noindex kaldırılıp Search Console’a sitemap gönderilecek
- [x] Düşük bulgu denetimi: `32` kayıt ayrı ayrı incelendi; `25` doğrulandı, `6` kısmen doğrulandı, `1` yanlış pozitif çıktı. Taksonomisiz `get_terms()` canlı WordPress 7.1 çağrısında `null` filtre argümanıyla yeniden üretildi ve kapatıldı; doğrulama envanteri `13 grup / 111 satır / 152 kontrol`
- [x] Düşük bulgu regresyon kapısı: yeni `18/18` güvenlik/uyumluluk kontrolü PASS; taksonomisiz terim sorgusu `null-safe`, XML-RPC `403`, varlık önbelleği `max-age=31536000, immutable`, içerik seçeneği autoload `3/3`, müşteri e-postası taşıyan takip URL'i `0`
...
- [x] “Çok yakında” ve ana sayfa ortak responsive video çifti: `25,000 sn`; masaüstü en iyi kaynakla byte olarak aynı `1920×1080 / 13.660.330 bayt`, mobil `900×1600 / 7.136.721 bayt`, H.264 High / 24 fps / ses izi `0/0`; önceki aşırı sıkıştırılmış sürümlere göre kaynak sadakati masaüstünde `SSIM 0,9856→1,0000`, mobilde `0,9822→0,9920`, responsive poster ve `by Kübra Gültekin` katmanı korunuyor
...
- [x] EDM Bilişim e-Fatura / e-Arşiv entegrasyonu teknik sözleşmesi ve güvenlik katmanı tamamlandı: gerçek WSDL eşleşmesi, UBL-TR, EDM yanıtlı numaralandırma, fail-closed yönlendirme, poller, kurtarma, fulfillment kapısı, mükerrerlik ve sıfır sır sızıntısı (`docs/EDM_ENTEGRASYONU.md`)
- [x] EDM sandbox'ta bir `LoadInvoice` taslağı ve düzeltilmiş bir gerçek `SendInvoice` kabul edildi; gönderilen belge numaralandı ve EDM'de varlığı doğrulandı
- [ ] EDM sandbox belgesinin terminal `SEND - SUCCEED` durumu henüz gözlemlenmedi; son salt-okunur ölçüm `PACKAGE - PROCESSING`
- [x] EDM kodu ayrı `kuka-island-edm` eklentisine taşındı; teslim durumunda pasif, auto-send kapalı ve manuel fatura/kargo akışı korunuyor
- [x] Kargo otomasyonu ayrı `kuka-island-shipping-automation` eklentisi olarak kuruldu: taşıyıcıdan bağımsız sözleşme, DHL eCommerce Türkiye adaptörü, HPOS uyumlu sipariş metası, WooCommerce Fulfillments entegrasyonu ve sınırlı durum sorgusu zinciri. Teslim durumunda pasif, otomasyon kapalı, manuel kargo yolu korunuyor (`docs/DHL_ENTEGRASYONU.md`)
- [x] Kargo ölçümü: beş OpenAPI dosyası `5/5` SHA-256 doğrulandı ve 13 operasyon dokümanla eşleşti; `48` kargo kontrolü ve iki bağımsız `make verify` turu `2/2 PASS`, smoke `5/5`, PHPCS `133/133`, kargo DB keyset PRE=POST
- [ ] DHL sandbox `customerNumber`/`password` gelince salt-okunur Identity + CBS bağlantı testi yapılacak; şu an kimlik `2/4` ve dış çağrı `0`
- [ ] Ardından ayrı onayla tek sandbox gönderisi oluşturulup sorgulanacak ve iptal edilecek; aynı turda hangi değerin WooCommerce takip numarası olduğu ölçülecek
- [x] iyzico sandbox ödeme, callback/idempotency ve iade koruma akışları bağlandı ve doğrulandı
- [ ] Canlı EDM aktivasyonu, canlı iyzico ilk ödeme ve genel canlıya alma kontrolleri ilgili gerçek hesap bilgileri geldiğinde tamamlanacak
