...
- Meta açıklamaları ve paylaşım kartları panel/sayfa/kategori verisinden basılır; boş alan etiket üretmez. Yerel kurulumda sosyal paylaşım görseli ve kategori açıklamaları boştur, bu yüzden `og:image` yalnız ürün sayfasında, kategori sayfalarında meta açıklaması hiç çıkmaz. Lansman öncesi operatör Site Görünümü → SEO, sayfa düzenleyici ve kategori açıklamalarını doldurmalı, Ayarlar → Genel → Slogan girmelidir.
- Ana sayfa ve “çok yakında” videosu sayfa yüklendikten sonra boşta başlar; `effectiveType` 4g dışını bildiren tarayıcıda (2G/3G) hiç indirilmez, poster kalır. Safari ve Firefox bu bilgiyi vermez, orada video her bağlantıda oynar. Bu bir lab kararıdır; gerçek cihaz Core Web Vitals turu hâlâ açık.
