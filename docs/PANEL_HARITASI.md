...
- Site Görünümü: **13 sekme, 115 görünen alan satırı, 157 saklanan alan kontrolü**. Fark, 42 Türkçe alanın aynı satırdaki `(EN)` karşılığıdır.
...
| 8. Footer | Bülten alanı, yardım/yasal linkler ve ödeme güven şeridi | `newsletter_enabled: checkbox`; `newsletter_eyebrow: text`; `newsletter_eyebrow_en: text`; `newsletter_title: text`; `newsletter_title_en: text`; `newsletter_copy: textarea`; `newsletter_copy_en: textarea`; `newsletter_consent: textarea`; `newsletter_consent_en: textarea`; `newsletter_notification_email: email`; `payment_logos_enabled: checkbox`; `payment_label: text`; `payment_label_en: text`; `help_links: link_lines`; `help_links_labels_en: lines`; `legal_links: link_lines`; `legal_links_labels_en: lines` |
